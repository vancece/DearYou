...
> 用 AI 替你写一封给阿嬷的家书。项目本身很简单，但从模型调用、云函数、数据库、静态托管到一键部署，全部基于云开发，和 CodeBuddy 使用云开发提供的大模型完成。
> 本文不以功能为重点，而是以工程化思维为线索，拆解一个 AI 应用从构思到上线的完整链路。
...
其中 **CodeBuddy 原生集成了云开发**，开箱即用，可以直接部署云资源；其他工具如 Cursor、Claude Code、Codex、Trae、Cline、GitHub Copilot 等，只需配置 Base URL 和 API Key 即可接入。
...
AI 应用部署涉及多个步骤：云函数部署、环境变量注入、前端构建、静态托管上传。这些步骤单独看不复杂，但组合起来容易遗漏。
