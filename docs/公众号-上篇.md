> 用 AI 替你写一封给阿嬤的家书。项目本身很简单，但从模型调用、云函数、数据库、静态托管到一键部署，全部基于云开发，AI 工具使用云开发提供的大模型完成。
...
- **模型调用** 
前端不能直接暴露模型 Key，需要服务端代理。

- **Prompt 编排** 
不同关系、不同风格、不同输入内容，需要在服务端统一拼装请求。

- **结构化输出** 
模型返回内容需要可解析，不能只返回一段自然语言。

- **数据持久化** 
生成后的信件需要入库，才能被分享页和树洞再次读取。
...
云开发 AI 网关兼容[主流 AI 协议](https://docs.cloudbase.net/ai/ai-tools/protocol)，调用方式接近常见的大模型 API：配置 Base URL、API Key、Model Name 即可请求。
...
- **CodeBuddy**把云资源部署编排进开发工作流，从"代码写好了"到"项目上线了"通过一条指令完成。
...
[案例代码](https://github.com/vancece/DearYou)
