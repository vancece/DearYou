...
| 变量名 | 用途 | 示例 |
|--------|------|------|
| `VITE_CLOUDBASE_ENV_ID` | 前端 SDK 初始化 | `your-env-id` |
| `VITE_CLOUDBASE_REGION` | 区域 | `ap-shanghai` |
| `VITE_CLOUDBASE_ACCESS_KEY` | 前端 Publishable Key + AI API Key | `your-access-key` |

- 如果 `.env` 不存在，从 `.env.example` 复制并提示用户填写。
- 如果缺少必要配置，列出缺少的项目并询问用户提供。
- `VITE_CLOUDBASE_ACCESS_KEY` 同时作为云函数 `loveletter` 的 AI API Key（环境变量名 `CLOUDBASE_AI_API_KEY`），向用户确认。
...
☁️ 云函数（5 个）：
  - loveletter（AI 生成信件，超时 60s，需要 API Key）
...
2. `loveletter` 函数需要设置环境变量 `CLOUDBASE_AI_API_KEY`，值从 `.env` 的 `VITE_CLOUDBASE_ACCESS_KEY` 读取
