const cloudbase = require('@cloudbase/node-sdk');

const ENV_ID = process.env.TCB_ENV || process.env.SCF_NAMESPACE;
const app = cloudbase.init({ env: ENV_ID });
const db = app.database();

exports.main = async (event = {}) => {
  try {
    const input =
      typeof event.body === 'string' ? JSON.parse(event.body || '{}') : event.body || event;

    const { id } = input;
    if (!id) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json; charset=utf-8', 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ ok: false, error: '缺少信件 ID' }),
      };
    }

    const { data } = await db.collection('letters').doc(id).get();
    if (!data || (Array.isArray(data) && data.length === 0)) {
      return {
        statusCode: 404,
        headers: { 'Content-Type': 'application/json; charset=utf-8', 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ ok: false, error: '信件不存在' }),
      };
    }

    const letter = Array.isArray(data) ? data[0] : data;

    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json; charset=utf-8', 'Access-Control-Allow-Origin': '*' },
      body: JSON.stringify({ ok: true, ...letter }),
    };
  } catch (err) {
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json; charset=utf-8', 'Access-Control-Allow-Origin': '*' },
      body: JSON.stringify({ ok: false, error: String(err && err.message ? err.message : err) }),
    };
  }
};
