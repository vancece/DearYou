...
本项目配套了两篇拆解工程实现的公众号文章，从产品构思到上线的完整链路，以及云开发在 AI 应用中的工程化实践：

- [给阿嬤一封来自云端的信（上）](docs/公众号-上篇.md) —— 用工程化思维拆解一个 AI 应用从构思到上线的完整链路：模型调用、云函数、数据库、静态托管到一键部署。
- [给阿嬤一封来自云端的信（下）](docs/公众号-下篇.md) —— 继续拆解工程化实现细节：结构化输出、服务端代理、数据持久化、内容安全、AI 网关，以及通过 AI 工具以一条指令完成部署。
